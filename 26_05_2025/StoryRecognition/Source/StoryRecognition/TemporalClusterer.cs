...
namespace StoryRecognition
{
    ...
    public class TemporalClusterer: IClusterer
    {
        public string Name => "TemporalClusterer";

        int _threshold;
        bool _writeClusters;
        string _writeDestination;

        public TemporalClusterer(int secondsThreshold = 5, bool writeClusters = false, string writeDestination = "")
        {
            _threshold = secondsThreshold;
        ...
        {
            List<LogEvent> sorted = events.OrderBy(e => e.Timestamp).ToList();
            ...
            foreach (LogEvent log in sorted)
            {
                ...
                int timeDiff = log.Timestamp - last.Timestamp;
